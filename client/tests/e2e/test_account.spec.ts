...
test('test create a new account', async ({ page }) => {
  ...
  await page.getByPlaceholder('josiah_carberry@brown.edu').fill('test_new4@brown.edu');
  ...
  await expect(page.locator('#root')).toContainText('Testing testBeginner ChefDiet:PescetarianIntolerances:NutCooking for 6Update Account');
  await page.locator('div').filter({ hasText: /^Liked Recipes:$/ }).click();
  await expect(page.locator('#root')).toContainText('Liked Recipes:Disliked Recipes:');
});
