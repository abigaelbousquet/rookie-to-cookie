...
public class IntegrationTests {
    ...
    public void testFullPipeline() throws IOException {
        HttpURLConnection clientConnection = tryRequest("generate-mealplan?uid=test&diet=Pescetarian&intolerances=Peanut&daysOfWeek=wednesday&cuisine=&servings=1&max_time=20&mode=personalize");
        // Get an OK response (the *connection* worked, the *API* provides an error response)
        assertEquals(404, clientConnection.getResponseCode());

        clientConnection.disconnect();

        HttpURLConnection clientConnection1 = tryRequest("save-mealplan?uid=2YtRmY2yYWO1OKarSPlFqtzdi6T2&dateOfMonday=06%2F10%2F2024");
        // Get an OK response (the *connection* worked, the *API* provides an error response)
        assertEquals(404, clientConnection.getResponseCode());
