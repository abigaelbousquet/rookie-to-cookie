...
public class TestGeneratorUtilities {
    ...
    public void testPickTop() {
        List<Recipe> sortedOptions = Arrays.asList(this.recipe1, this.recipe3, this.recipe2);
        int numRecommendations = 2;
        List<Recipe> topRecipes = GeneratorUtilities.pickTop(sortedOptions, numRecommendations);
        Assert.assertEquals(2, topRecipes.size());
    }

    @Test
    public void testFilterGoodRatings() {

        List<Recipe> allRecipes = Arrays.asList(this.recipe1, this.recipe2);
        List<Recipe> filteredRecipes = GeneratorUtilities.filterGoodRatings(allRecipes);
        Assert.assertEquals(2, filteredRecipes.size());
        Assert.assertEquals(this.recipe1, filteredRecipes.get(0));
    }

    @Test
    public void testGetNearestNeighborsToListRecipes() {
        List<Recipe> candidates = Arrays.asList(this.recipe1, this.recipe2, this.recipe3);
        List<Recipe> history = Arrays.asList(this.recipe4, this.recipe5);
        int n = 2;
        PriorityQueue<RecipeFrequencyPair> nearestNeighbors = GeneratorUtilities.getNearestNeighborsToListRecipes(candidates, history, n);
        Assert.assertEquals(2, nearestNeighbors.size());
    }
